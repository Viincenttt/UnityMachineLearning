...
public class PopulationManager : MonoBehaviour {
    ...
    public static int numberOfBirdsPassed = 0;
    ...
    void BreedNewPopulation() {
	    ...
	    List<Brain> population = this.GetCurrentPopulation().OrderByDescending(x => x.DistanceTravelled).ToList();
    ...
    void Update () {
		...
		if(elapsed >= this._trialTime) {
			this.BreedNewPopulation();
			elapsed = 0;
		    numberOfBirdsPassed = 0;
        }
	}
    ...
    private void OnGUI() {
        ...
        GUI.Label(new Rect(10, 50, 200, 30), $"Time: {elapsed:0.00}", this._guiStyle);
        GUI.Label(new Rect(10, 75, 200, 30), $"Score: {numberOfBirdsPassed}", this._guiStyle);
        GUI.EndGroup();
    }
}
